...
from apps.usuarios.models import Usuario, Padre, PersonalAdministrativo
...
def crear_usuario(nombre_usuario: str, contrasena_plana: str) -> Usuario:
    ...
    return Usuario.objects.create(
        ...
        contrasena=hashpass,
        correo=salt   # siguiendo tu diseño, 'correo' guarda el salt
    )
...
def selector_registro(request):
    return render(request, 'estudiantes/selector.html')
...
def registrar_regular(request):
    if request.method == 'POST':
        ...
        cuenta_form= CuentaForm(request.POST, prefix='cuenta')  # credenciales del padre

        if padre_form.is_valid() and est_form.is_valid() and cuenta_form.is_valid():
            # 1) Crear/validar Usuario (padre)
            try:
                ...
                return render(request, 'estudiantes/registro_regular.html', {
                    'padre_form': padre_form, 'est_form': est_form, 'cuenta_form': cuenta_form
                })

            # 2) Crear Perfil Padre
            padre = Padre.objects.create(
                usuario=u_padre,
                **padre_form.cleaned_data
            )

            # 3) Crear Estudiante tipo regular. IMPORTANTE:
            #    Según tu requerimiento, el "usuario" del estudiante (FK) será el del PADRE,
            #    de modo que el acceso lo hace el padre.
            est_data = est_form.cleaned_data
            Estudiante.objects.create(
                usuario=u_padre,   # <- login lo hace el padre
...
def registrar_tecnico(request):
    if request.method == 'POST':
        ...
        cuenta_form = CuentaForm(request.POST, prefix='cuenta')  # credenciales del estudiante
        ...
        if est_form.is_valid() and cuenta_form.is_valid():
            ...
            try:
                ...
                messages.error(request, str(e))
                return render(request, 'estudiantes/registro_tecnico.html', {
                    'est_form': est_form, 'cuenta_form': cuenta_form
                })

            # 2) Crear Estudiante tipo técnico (sin padre)
